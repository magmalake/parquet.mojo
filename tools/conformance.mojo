...
def _expected_unreadable(name: String) -> String:
    ...
    Both are deliberately corrupt and rejecting them is the correct answer.
    Naming them here rather than folding them into the pass count means a *new*
    failure stands out instead of blending into a known-bad list — and an entry
    that starts reading is reported as an `UNEXPECTED PASS` and fails the run,
    which is how this list stays honest in the other direction too.
    """
